...
from utils import get_int, gini
...
class CommonProfile:
    def __init__(self):
        ...
        self.feature_func_map = {
            ...
            'gini_target_pkts':
                lambda x: gini(np.unique(x['target_pkts'], return_counts=True)[1]),
            'gini_opposite_pkts':
                lambda x: gini(np.unique(x['opposite_pkts'], return_counts=True)[1]),
            'gini_target_bytes':
                lambda x: gini(np.unique(x['target_bytes'], return_counts=True)[1]),
            'gini_opposite_bytes':
                lambda x: gini(np.unique(x['opposite_bytes'], return_counts=True)[1]),
            'in_bytes_per_pkt':
            ...
            'gini_opposite_ip':
                lambda x: gini(np.unique(x['opposite_ip'], return_counts=True)[1]),
            'gini_opposite_port':
                lambda x: gini(np.unique(x['opposite_port'], return_counts=True)[1]),
            'gini_target_port':
                lambda x: gini(np.unique(x['target_port'], return_counts=True)[1]),
            'gini_pattern':
                lambda x: sorted(zip(*np.unique("{}_{}_{}_{}".format(x['target_pkts'], x['opposite_pkts'],
                                                                     x['target_bytes'], x['opposite_bytes']),
            ...
            'gini_int':
                lambda x: gini(get_int(x['start_time'], x['end_time'])) if np.sum(
                    get_int(x['start_time'], x['end_time'])) != 0 else 0
        }

        self.feature_list = list(self.feature_func_map.keys())